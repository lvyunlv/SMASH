...
OUTPUT_FILE="$RESULTS_DIR/qwen_nonlinear_semi.txt"
    ...
    for func in "${!FUNC_DIM_SIZE[@]}"; do
        ...
        for n in "${NS[@]}"; do
            for net in "${NETS[@]}"; do
                ...
                for ((i=1;i<=n;i++)); do
                    LOG_FILE="$LOG_DIR/qwen_nonlinear_semi_${func}_t${t}_n${n}_net${net}_party${i}.log"

                    "$BIN_DIR/test_lvt_semi" "$i" "$PORT" "$n" "$net" \
                        > "$LOG_FILE" 2>&1 &
