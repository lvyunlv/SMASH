# Build SMASH with all native deps inside a single container
FROM ubuntu:22.04

ENV DEBIAN_FRONTEND=noninteractive

# Base toolchain + crypto/math deps required by OpenFHE and EMP
...
# Build and install OpenFHE (uses the included source tree)
...
# Build and install the bundled EMP toolkits so CMake find_package() can locate them
