...
int main(int argc, char** argv){
    ...
    if(argc<5){cout<<"Format: <PartyID> <port> <num_parties> <network_condition> [ip_file]"<<endl; return 0;}
    ...
    for(int i=0;i<l;i++){x_bits[i]=tiny.distributed_share(tiny.rng()%2);}

    double total_time=0,total_comm=0,online_time=0,online_comm=0;
    int times=1;
    for(int i=0;i<times;i++){
        auto shared_x = B2A_spdz2k::B2A(elgl,lvt,tiny,spdz2k,party,num_party,io,&pool,FIELD_SIZE,x_bits,online_time,online_comm,is_wan);
